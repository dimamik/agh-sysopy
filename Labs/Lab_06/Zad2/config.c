...
mqd_t get_queue_by_name(char *name) {
    ...
    if ((queue_descriptor = mq_open(name, O_WRONLY)) == -1) {
        fprintf(stderr, "Error in get_queue_by_name: %s\n", strerror(errno));
        exit(EXIT_FAILURE);
    }
    return queue_descriptor;
}
...
void unlink_queue(char *name) {
    if (mq_unlink(name) == -1) {
        fprintf(stderr, "Error in unlink_queue: %s\n", strerror(errno));
        exit(EXIT_FAILURE);
    }
}
...
void close_queue(mqd_t queue_descr) {
    if (mq_close(queue_descr) == -1) {
        fprintf(stderr, "Error in close_queue: %s\n", strerror(errno));
        exit(EXIT_FAILURE);
    }
}
...
void add_notification(mqd_t queue_descr, struct sigevent *s_sigevent) {
    if (mq_notify(queue_descr, s_sigevent) == -1) {
        fprintf(stderr, "Error in add_notification: %s\n", strerror(errno));
        exit(EXIT_FAILURE);
    }
}
...
void get_message_from_queue(mqd_t queue_descr, char *msg, unsigned int *type) {
    if (mq_receive(queue_descr, msg, MAX_MESSAGE_TEXT_LENGTH, type) == -1) {
        fprintf(stderr, "Error in get_message_from_queue: %s\n", strerror(errno));
        exit(EXIT_FAILURE);
    }
}